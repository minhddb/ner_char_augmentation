...
def generate_data(data: Dataset, tags_column, ratios, stratified_samples=True, seed=0):
    ...
    for ratio in ratios:
        # Resample data for low-resource tests
        ...
        label2id = augmentation.label2id
        if ratio != 1:
            non_augmentation_samples = [{"tokens": sequence[0], 
                                         tags_column: [label2id[t] for t in sequence[1]]
                                         } for sequence in augmentation.samples]
            # Get all sentences with out any entity labels
            _non_entities_samples = [data[i] for i, tags in enumerate(data[tags_column]) 
                                     if all(tag == label2id["O"] for tag in tags)]
            # Resample sentences wo enttities using provided ratio: 0.1 should return the first 10% of all non-entity sentences
            non_entities_samples = _non_entities_samples[:round(ratio * len(_non_entities_samples))]

            data_wo_aug = concatenate_datasets([Dataset.from_list(non_entities_samples), 
                                                   Dataset.from_list(non_augmentation_samples)]
                                                   ).cast(data.features)
            print(f"Number of resampled entity sequences: {len(non_augmentation_samples)}")
            print(f"Number of non-entity sequences: {len(non_entities_samples)}")
        else:
            data_wo_aug = data
        data_wo_aug = data_wo_aug.shuffle(seed=seed)
        for aug in ["no_augmentation", "reverse_letter_case", "delete_character", "swap_characters", "substitute_character", "all"]:
            if aug == "no_augmentation":
                print(f"Data size wo augmentation: {len(data_wo_aug["tokens"])}\n")
                yield ratio, "no_augmentation", data_wo_aug
            else:
                if aug != "all":
                ...
                print(f"Data size wo augmentation: {len(data_wo_aug["tokens"])}")
                print(f"Augmented instances: {len(augmented)}")
                aug_dataset = Dataset.from_list(augmented) 
                # Insert noise injected instances to existing dataset with concatenation
                if "id" not in data_wo_aug.features.keys():
                    aug_dataset = aug_dataset.remove_columns("id")
                casted_augmented_dataset = aug_dataset.cast(data_wo_aug.features).shuffle(seed=seed)
                # Shuffle the original and augmented datasets seperately before concatenating them together
                data_w_aug = concatenate_datasets([data_wo_aug, casted_augmented_dataset])
                print(f"Data size w augmentation: {len(data_w_aug["tokens"])}\n")
                yield ratio, aug, data_w_aug
            
if __name__=="__main__":
    args = arguments()
    # args.hf_dataset = "wnut17"
    ...
    if args.stratified_samples:
        path_results = f"results/lr_{args.hf_dataset}_{args.model_name.split("/")[1].lower()}_stratified_samples.tsv"    
        output_dir = f"lr_trained_{args.model_name.split("/")[1].lower()}_stratified" 
